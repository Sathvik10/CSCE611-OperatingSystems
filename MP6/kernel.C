...
/* Macro to enable blocking disk. */
#define _BLOCKING_DISK_

/* Macro to test thread safe fucntionality*/
#define _THREAD_SAFE_DISK

/* Macro to enable mirror disk. */
// #define _MIRROR_DISK_
...
#ifdef _THREAD_SAFE_DISK
        Console::puts("Reading a block in FUN3 from disk...\n");
        SYSTEM_DISK->read(read_block, buf);
#endif
        for (int i = 0; i < 10; i++)
        {
