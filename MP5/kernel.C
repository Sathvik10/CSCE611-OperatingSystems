...
/* Function to test the RR Scheduler with terminating function.*/
void testingRRSchedular();
...
#ifdef _RR_SCHEDULER_
    RRSchedular *rrsched = new RRSchedular(50);
    SYSTEM_SCHEDULER = rrsched;
    InterruptHandler::register_handler(0, rrsched);
#endif
...
void fun5()
{
    ...
    for (int j = 0; j < 10; j++)
    {
        Console::puts("FUN 5 IN SLOW BURST[");
        Console::puti(j);
        Console::puts("]\n");
...
void fun6()
{
    ...
    Console::puti(Thread::CurrentThread()->ThreadId());
    Console::puts("\n");
    Console::puts("FUN 6 INVOKED!\n");

    for (int j = 0; j < 10; j++)
    {
        Console::puts("FUN 6 IN SLOW BURST[");
        Console::puti(j);
        Console::puts("]\n");
...
{
    char *stack1 = new char[1024];
    Thread *thread_1 = new Thread(fun5, stack1, 1024);

    char *stack2 = new char[1024];
    Thread *thread_2 = new Thread(fun6, stack2, 1024);

    SYSTEM_SCHEDULER->add(thread_2);

    Thread::dispatch_to(thread_1);
}
